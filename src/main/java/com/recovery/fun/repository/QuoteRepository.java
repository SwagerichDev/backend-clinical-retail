...
//@Query("SELECT SUM(p.price) FROM Quote c INNER JOIN c.procedure p WHERE TO_CHAR(c.datee, 'YYYYMMDD') = ?1")
    ...
    @Query("SELECT q.idQuote as idQuote, q.status as status,q.datee as datee, p.name as name,p.lastName as lastName,p.dni as dni,pro.idProcedure as idProcedure,pro.name as procedureName FROM Quote " +
            "q  JOIN  q.patient p  join  q.procedure pro where p.idPatient =?1")
